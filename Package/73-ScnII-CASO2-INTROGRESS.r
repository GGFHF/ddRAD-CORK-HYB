...
ScnII<-read.table("ScnII-CASO2.stru",header=TRUE, sep="")
...
ScnII.CASO2<-prepare.data(admix.gen=ScnII.HY.gt, 
             loci.data=loci.data,
...
hybrid.index<-est.h(introgress.data=ScnII.CASO2, 
                    loci.data=loci.data, 
                    ind.touse=NULL,
                    fixed=FALSE, 
                    p1.allele=NULL, 
...
write.csv(hybrid.index,"ScnII-CASO2-hybrid_index.csv")
