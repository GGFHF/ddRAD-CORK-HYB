...
ScnI<-read.table("ScnI-CASO1.stru",header=TRUE, sep="")
...
ScnI.CASO1<-prepare.data(admix.gen=ScnI.HY.gt, 
             loci.data=loci.data,
...
hybrid.index<-est.h(introgress.data=ScnI.CASO1, 
                    loci.data=loci.data, 
                    ind.touse=NULL,
                    fixed=FALSE, 
                    p1.allele=NULL, 
...
write.csv(hybrid.index,"ScnI-CASO1-hybrid_index.csv")
