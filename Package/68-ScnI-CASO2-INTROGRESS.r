...
ScnI<-read.table("ScnI-CASO2.stru",header=TRUE, sep="")
...
ScnI.CASO2<-prepare.data(admix.gen=ScnI.HY.gt, 
             loci.data=loci.data,
...
hybrid.index<-est.h(introgress.data=ScnI.CASO2, 
                    loci.data=loci.data, 
                    ind.touse=NULL,
                    fixed=FALSE, 
                    p1.allele=NULL, 
...
write.csv(hybrid.index,"ScnI-CASO2-hybrid_index.csv")
