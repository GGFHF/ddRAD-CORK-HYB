...
# This script merges all individuals vcf files obtained from the alignment to pseudogenome
...
echo 'RUNNING BCFTOOLS TO MERGE VCF.GZ FILES...'
    ...
    $BCFTOOLS/bcftools merge --merge all -O v $DATA_DIR/*vcf.gz > $OUTPUT_DIR/MERGED-PSEUDOGENOME.vcf
    if [ $? -ne 0 ]; then echo 'Script ended with errors.'; exit 1; fi
