...
ScnIV<-read.table("ScnIV-CASO1.stru",header=TRUE, sep="")
...
ScnIV.CASO1<-prepare.data(admix.gen=ScnIV.HY.gt, 
             loci.data=loci.data,
...
hybrid.index<-est.h(introgress.data=ScnIV.CASO1, 
                    loci.data=loci.data, 
                    ind.touse=NULL,
                    fixed=FALSE, 
                    p1.allele=NULL, 
...
write.csv(hybrid.index,"ScnIV-CASO1-hybrid_index.csv")
