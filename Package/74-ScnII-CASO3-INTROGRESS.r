...
ScnII<-read.table("ScnII-CASO3.stru",header=TRUE, sep="")
...
ScnII.CASO3<-prepare.data(admix.gen=ScnII.HY.gt, 
             loci.data=loci.data,
...
hybrid.index<-est.h(introgress.data=ScnII.CASO3, 
                    loci.data=loci.data, 
                    ind.touse=NULL,
                    fixed=FALSE, 
                    p1.allele=NULL, 
...
write.csv(hybrid.index,"ScnII-CASO3-hybrid_index.csv")
