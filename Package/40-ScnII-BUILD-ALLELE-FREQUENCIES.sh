...
DATA_DIR=PATH_TO_YOUR_DATA_DIRECTORY		# where ScnII-PROGENIES-IMPUTED.vcf and IDs-total.txt file are stored
...
/usr/bin/time \
    ./build-allele-frequency.py \
        --vcf=$DATA_DIR/ScnII-PROGENIES-IMPUTED.vcf \
        --samples=$DATA_DIR/IDs-total.txt \
        --sp1_id=AL \
        --sp2_id=EN \
        --hyb_id=HY \
        --out=$OUTPUT_DIR/ScnII-PROGENIES-IMPUTED-allelefreq.csv \
        --varnum=1000 \
        --trans=ADD100 \
        --verbose=Y \
        --trace=N \
